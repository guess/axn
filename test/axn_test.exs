defmodule AxnTest do
  ...
  describe "Axn DSL basic compilation" do
    test "basic module compilation and telemetry_prefix" do
      ...
      assert Code.ensure_loaded?(BasicCompilationModule)
      # Run function is generated
      assert function_exported?(BasicCompilationModule, :run, 3)
      # Empty action works
      assert {:ok, nil} = BasicCompilationModule.run(:simple, %{}, %{})
      # Undefined action errors
      assert {:error, :action_not_found} = BasicCompilationModule.run(:missing, %{}, %{})
    end
  end
